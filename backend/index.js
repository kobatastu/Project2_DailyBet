...
app.get('/', (req, res) => {
    ...
    connection.query('SELECT * from Bet_table',
...
app.get('/user', (req, res) => {
    ...
    connection.query('SELECT * from User_table WHERE name=?',name,
    function(error,results,fields){
        if(results[0] == null){
            data = {'name':name,'coin':100,'nicname':'No name','mypic':'default.png'}
            ...
            function(error2,results2,field2){
                ...
                connection.query('SELECT * FROM Bet_table RIGHT OUTER JOIN Coin_table ON Coin_table.betid=Bet_table.id WHERE userid = ? and kanri = 0 and status != "X"',name,
                function(error3,results3,fields3){
                    ...
                    for (let i =0;i<results3.length;i++){
                        ...
                        if (results3[i].status==results3[i].which && results3[i].which=="A"){
                            let a = (results3[i].A_total+results3[i].B_total)*11/10+100
                        ...
                        } else if (results3[i].status==results3[i].which && results3[i].which=="B") {
                            let a = (results3[i].A_total+results3[i].B_total)*11/10+100
                    ...
                    connection.query('UPDATE User_table SET ? WHERE name=?',[cv,name],
                    function(error4,results4,fields4){
                        ...
                        for(let i = 0;i<array.length;i++){
                            if(i==array.length-1){
                                ...
                                connection.query('UPDATE Coin_table SET kanri=1 WHERE id = ?',array[i],
...
app.get('/mybet', (req, res) => {
    ...
    connection.query('SELECT * from Bet_table LEFT OUTER JOIN Coin_table ON Coin_table.betid = Bet_table.id WHERE userid = ? ORDER BY Coin_table.id DESC',name,
    function(error,results,fields){
...
app.get('/board', (req, res) => {
    ...
    connection.query('SELECT * from User_table RIGHT OUTER JOIN Board_table ON Board_table.userid=User_table.name',
    function(error,results,fields){
...
app.post('/upload/name/',(req,res) => {
    ...
    const connection =mysql.createConnection(mysql_setting);
    connection.connect();
    connection.query('UPDATE User_table set ? WHERE name = ?',[data,name],
    function(error,results,fields){
        res.send(data);
...
app.post('/',(req,res)=>{
    ...
    const connection =mysql.createConnection(mysql_setting);
    connection.connect();
    connection.query('INSERT INTO Coin_table set ?',data,
    function(error,results,fields){
        if(name=="A"){
            const newA = Number(Atotal) + Number(bet)
            const data1 = {'A_total':newA}
            connection.query('UPDATE Bet_table SET ? WHERE id = ?',[data1,id],
            function(error2,results2,fields2){
                connection.query('UPDATE User_table SET coin = coin - ? WHERE name = ?',[Number(bet),userid],
                function(error3,results3,fields3){
...
app.post('/buycoin',(req,res) => {
    ...
    const connection =mysql.createConnection(mysql_setting);
    connection.connect();
    connection.query('UPDATE User_table SET coin = coin + ? WHERE name = ?',[Number(coin),name],
    function(error,results,fields){
        console.log(error)
        res.send(data)
    })
...
// クライアントからサーバーへ メッセージ送信ハンドラ（自分を含む全員宛に送る）
  socket.on('SEND_MESSAGE', function(value){
    ...
    function(error,results,fields){
        connection.query('SELECT * FROM User_table WHERE name = ?',value.userid,
        function(error2,results2,field2){
          
            const data1 = {'id':results.insertId,'nicname':results2[0].nicname,'mypic':results2[0].mypic,'userid':value.userid,'contents':value.contents,'time':date}
