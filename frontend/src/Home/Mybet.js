...
class Mybet extends Component {
    ...
    async componentDidMount(){
        ...
        const bets = (await axios.get('http://localhost:8080/mybet?email='+this.props.myemail)).data;
        this.setState({
            bets,
        })
    }
                        ...
                        <Card style={{maxWidth:500,margin:'50px auto',backgroundColor:'white'}}>
                            ...
                            <CardContent>
                                ...
                                <div style={{textAlign:'center'}}>
                                    {bet.which==='A' && <p><b>{bet.Aname}</b>に<b>{bet.betvalue}コイン</b>賭けました</p>}
                                    {bet.which==='B' && <p><b>{bet.Bname}</b>に<b>{bet.betvalue}コイン</b>賭けました</p>}
                                </div>
                                                              
                                <div style={{textAlign:'center'}}>
                                    {bet.status==='X' && <p>勝敗はまだ決していません</p>}
                                    {bet.status==='A' && <p><b>{bet.Aname}</b>の勝利です</p>}
                                    {bet.status==='B' && <p><b>{bet.Bname}</b>の勝利です</p>}
                                </div>
